...
export const getMistakesDB = async () => {
  ...
  try {
    ...
    if (initAttempts < MAX_INIT_ATTEMPTS) {
      console.log(`🚫 Mistakes DB init failed, retrying... (attempt ${initAttempts}/${MAX_INIT_ATTEMPTS})`);
      // Wait a bit before retrying
      await new Promise(resolve => setTimeout(resolve, 1000 * initAttempts));
      return getMistakesDB(); // Retry
    }
...
const initializeDatabase = async () => {
  try {
    console.log('🚫 Initializing mistakes database...');
    ...
    const db = await SQLite.openDatabaseAsync("mistakes.db", {
...
export const openMistakesDB = getMistakesDB;

// Database health check
export const checkMistakesDatabaseHealth = async () => {
  try {
    const db = await getMistakesDB();
    ...
    const count = await db.getFirstAsync("SELECT COUNT(*) as count FROM mistakes");
    ...
    console.error('❌ Mistakes database health check failed:', error);
    return {
      healthy: false,
      error: error.message,
...
export const insertMistakesEntry = async ({ title = "", mistake, solution, category, created_at, updated_at }) => {
  try {
    const db = await getMistakesDB();
...
export const updateMistakesEntry = async ({ id, title, mistake, solution, category, updated_at }) => {
  try {
    const db = await getMistakesDB();
    await db.runAsync(
...
export const markMistakesEntrySynced = async ({ id, server_id, server_meta = null }) => {
  try {
    const db = await getMistakesDB();
...
export const upsertMistakesFromServer = async ({ server_id, title, mistake, solution, category, created_at, updated_at, server_meta }) => {
  try {
    const db = await getMistakesDB();
    const metaJson = server_meta ? JSON.stringify(server_meta) : null;
    ...
    if (existing) {
      ...
      await db.runAsync(
        ...
        [title, mistake, solution, category, created_at, updated_at, metaJson, server_id]
      );
    } else {
      ...
      await db.runAsync(
        ...
        [title, mistake, solution, category, created_at, updated_at, server_id, metaJson]
      );
    }
