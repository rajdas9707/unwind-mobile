...
class BuyItemsStorage {
  ...
  validateItemName(name) {
    ...
    if (trimmedName.length > 100) {
      return { isValid: false, error: 'Item name is too long (max 100 characters)' };
    }
  ...
  validateLocation(location) {
    ...
    if (trimmedLocation.length > 100) {
      return { isValid: false, error: 'Location is too long (max 100 characters)' };
    }
  ...
  async createItem(listId, name, location = '') {
    try {
      ...
      const itemId = await database.createItem(listId, nameValidation.name, locationValidation.location);
      return itemId;
    } catch (error) {
  ...
  async updateItem(itemId, name, location = '') {
    try {
      ...
      await database.updateItem(itemId, nameValidation.name, locationValidation.location);
      return true;
    } catch (error) {
  ...
  calculateCompletionPercentage(totalItems, boughtItems) {
...
export default new BuyItemsStorage();