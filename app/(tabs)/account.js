...
export default function AccountScreen() {
  ...
  const loadUserStats = async () => {
    try {
      ...
      const overthinkingEntries = await AsyncStorage.getItem('overthinkingEntries');
      ...
      const overthinkingCount = overthinkingEntries ? JSON.parse(overthinkingEntries).length : 0;
      const mistakeCount = mistakeEntries ? JSON.parse(mistakeEntries).length : 0;

      setStats({
        journalEntries: journalCount,
        overthinkingLogs: overthinkingCount,
        mistakeEntries: mistakeCount,
        streaks: Math.floor((journalCount + overthinkingCount + mistakeCount) / 7),
      });
    } catch (error) {
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await AsyncStorage.multiRemove(['journalEntries', 'overthinkingEntries', 'mistakeEntries']);
              loadUserStats();
          ...
          <View style={styles.profileInfo}>
            {isEditing ? (
              ...
              <View style={styles.profileDetails}>
                ...
                <Text style={styles.joinDate}>Member since {new Date(userInfo.joinDate).toLocaleDateString()}</Text>
              </View>
            )}
          </View>
        </View>
